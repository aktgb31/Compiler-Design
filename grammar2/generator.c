...
void addSymbol(struct symbol *table, char *name, int *pointer)
{
    struct symbol *newSymbol = (struct symbol *)malloc(sizeof(struct symbol));
    newSymbol->name = name;
    newSymbol->binding = ++(*pointer);
...
struct tnode *makeLeafVar(char *var)
{
    ...
    struct symbol *table = (localScope == 0) ? GSymbolTable : LSymbolTable;
    temp->entry = findSymbol(table, temp->varName);
    if (temp->entry == NULL)
    {
        addSymbol(table, temp->varName, (localScope == 0 ? &SP : &BP_OFFSET));
        temp->entry = findSymbol(table, temp->varName);
    }
    return temp;
}
...
int evaluate(struct tnode *t, FILE *targetFile)
{
    ...
    else if (t->nodeType == NTFUNCALL)
    {
        ...
        fprintf(targetFile, "POP R%d\n", j); // Pop return value
        ++IP;
        freeReg(); // Freed the newly allocated registor

        fprintf(targetFile, "POP R0\n"); // Pop arguments

        popUsedRegistors(targetFile);
        freeReg(); // freed the registor that contained argument
        i = getReg();
        if (i != j)
        {
            printf("Error Ambiguous Return Value\n");
            exit(1);
        }
        return i;
    }
